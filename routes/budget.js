const express = require('express');
  ...
  router.get('/', async (req, res) => {
      try {
          ...
          console.error('Error fetching budget items:', err.stack);
  ...
  router.post('/', async (req, res) => {
      ...
      try {
          ...
          console.error('Error adding budget item:', err.stack);
          res.status(500).json({ error: 'Internal server error' });
      }
  });
  ...
  router.put('/:id', async (req, res) => {
      ...
      try {
          ...
          console.error('Error updating budget item:', err.stack);
          res.status(500).json({ error: 'Internal server error' });
      }
  });
  ...
  router.delete('/:id', async (req, res) => {
      ...
      try {
          ...
          console.error('Error deleting budget item:', err.stack);
          res.status(500).json({ error: 'Internal server error' });
      }
  });
